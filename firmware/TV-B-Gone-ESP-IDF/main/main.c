...
static void start_tvb_gone_task(void);
 ...
 * Returns the number of symbols built, or 0 on error (e.g. buffer overflow).
 * ----------------------------------------------------------------------- */
static size_t build_power_code_symbols(rmt_symbol_word_t *symbols, size_t max_symbols,
                                       int numPairs, uint32_t *pairsTab_ptr,
                                       uint8_t *sequenceTab_ptr)
 ...
 * xmitCode()
 *
 * Transmit one full POWER-Code using the RMT TX driver.
 * ----------------------------------------------------------------------- */
static void xmitCode(uint32_t carrierFreq, int numPairs, uint32_t *pairsTab_ptr,
                     uint8_t *sequenceTab_ptr)
{
...
static void init_hardware(void)
{
    ...
    // Set the active-LOW built-in LED on GPIO8 as output, initially off (HIGH)
    gpio_set_level((gpio_num_t)VISLED, HIGH);
...
void app_main(void)
{
    // Initialize hardware with the board configuration (GPIOs, RMT channel, etc.)
    init_hardware();
    // Start the main TV-B-Gone task, which will run indefinitely on the FreeRTOS scheduler
    start_tvb_gone_task();
}
